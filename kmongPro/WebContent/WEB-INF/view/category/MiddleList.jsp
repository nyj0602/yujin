...
<form id="checkform">
			...
			<div class="row">
				...
				<div class="col-xs-2">
					...
					<div class="row margin-top-5">
						...
						<c:forEach items="${largeCategory }" var="dto">
							<li class="category-list-item root-category"><a
								class="plain font-color-light sendGAEventClz"
								href="/kmongPro/category/category.do?category=${dto.c_id }" data-name="사이드 메뉴 - 카테고리 선택 - 1">${dto.c_group }</a>
							</li>
						</c:forEach>
						</ul>
							...
							<!-- 해당 소분류 카테고리 리스트가 있으면 뿌려주는 부분 -->
							<ul class="margin-none list-unstyled">
								<c:forEach items="${categoryList }" var="dto2">
									<li class='padding-right-0 category-list-item sub-category'>
										<a class='plain font-color-light sendGAEventClz'
										href='/kmongPro/category/categorylist.do?category=${dto2.c_id }'
										data-name='사이드 메뉴 - 카테고리 선택 - ${dto2.c_id } '>${dto2.c_group }</a>
									</li>
								</c:forEach>
							</ul>
						</div>
					...
					<div class="padding-all-15">
						...
						<!-- 해당 카테고리의 체크박스 뿌려주기 -->
						<jsp:include page="/WEB-INF/view/inc/category_inc/checkbox.jsp">
							<jsp:param value="${checkboxName }" name="checkboxName"/>
							<jsp:param value="${checkboxList }" name="checkboxList"/>
								...
								<div class="select-control">
									<!-- <form method="get" id="form2"> -->
										<select class="input-xs" name="sort">
											<option id="recommendation" value="recommendation"
												${sort eq "recommendation"? "selected":""}>추천순</option>
											<option id="ranking" value="ranking"
												${sort eq "ranking"? "selected":""}>랭킹순</option>
											<option id="created_at" value="created_at"
												${sort eq "created_at"? "selected":""}>신규등록순</option>
										</select>
					...
					<jsp:include page="/WEB-INF/view/inc/category_inc/talent.jsp">
							...
							<div class="text-center margin-top-10 pagination-small">
								<ul class="pagination">
									...
									<li><a
										href="/kmongPro/category/categorylist.do?category=${category }&sort=${sort }&page=${(page-1)<1 ? 1:(page-1) }&grade=${grade }&check=${check}"
										rel="next">«</a></li>

									<c:forEach var="i" begin="1"
										end="${ size/12 +(1-(size/12%1))%1}">
										<!-- EL 올림 공식 -->
										<li><a
											href="/kmongPro/category/categorylist.do?category=${category }&sort=${sort }&page=${i }&grade=${grade }&check=${check}">${i }</a></li>
									</c:forEach>

									<li><a
										href="/kmongPro/category/categorylist.do?category=${category }&sort=${sort }&page=${page+1<=(size/12 +(1-(size/12%1))%1)? page+1:page }&grade=${grade }&check=${check}"
										rel="next">»</a></li>
								</ul>
							</div>
	...
	<!-- category_list_div -->
			<script>
					...
					$(function () {
						$("label>.sellerRatingCheckbox, label>.gigDataCheckbox, select.input-xs").change(function () {
							...
							$("#checkform").prop("action","/kmongPro/category/categorylist.do");
							$("#checkform").submit();
